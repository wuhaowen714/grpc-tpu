...
class Qwen:
    ...
    def chat_stream(self, input_str, history, system=''):
        input_tokens = make_context(self.sp, query=input_str, max_window_size=self.SEQLEN)
        if (len(input_tokens) > self.SEQLEN / 3):
            yield '##ERROR'
            return

        tokens = make_context(self.sp, query=input_str, 
                        history=history,
                        system=system,
